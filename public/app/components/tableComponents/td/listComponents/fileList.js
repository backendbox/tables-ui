...
class FileListComponent extends React.Component {
	...
	getFileIcon(file) {
		if (file.type) {
			...
			if (fileType) {
				if (['png', 'jpeg', 'jpg', 'gif'].indexOf(fileType) > -1) {
					return <img src={this.checkIfPrivateFile(file) ? '/app/assets/images/file/file.png' : file.url} className={this.checkIfPrivateFile(file) || this.props.isListOfRelation ? 'fsmimagenf' : 'fsmimage'} />
				} else if (CONFIG.iconTypes.indexOf(fileType) > -1) {
